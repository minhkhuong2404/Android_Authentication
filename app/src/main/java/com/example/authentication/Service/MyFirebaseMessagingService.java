...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    private void sendNotification(String messageBody) throws ParseException {
        ...
        if (messageBody.toLowerCase().contains("message")) {
            which_icon = getBaseContext().getResources().getIdentifier("message", "drawable", getApplicationContext().getPackageName());
        } else if (messageBody.toLowerCase().contains("video")) {
            which_icon = getBaseContext().getResources().getIdentifier("videocam", "drawable", getApplicationContext().getPackageName());
        } else if (messageBody.toLowerCase().contains("badge")) {
            which_icon = getBaseContext().getResources().getIdentifier("badge", "drawable", getApplicationContext().getPackageName());
        } else if (messageBody.toLowerCase().contains("information")) {
            which_icon = getBaseContext().getResources().getIdentifier("info", "drawable", getApplicationContext().getPackageName());
        } else {
            which_icon = getBaseContext().getResources().getIdentifier("noti", "drawable", getApplicationContext().getPackageName());
        }

        // add raw datetime to the notification
        Date date = new Date();
        String dateStr = inputFormat.format(date);
        notifHandler.addNotificationHandler(new NotificationItem(dateStr, messageBody, which_icon, dateStr));

        // load all notification items
        List<NotificationItem> notificationItems = notifHandler.loadNotificationHandler();
